...
void initializarePiesa(piesa &p)
{
    ...
    for (unsigned i=1; i<=p.nrNoduri; i++)
    {
        ...
        p.noduri[i].x=p.x+zoom1*p.noduri[i].d;
        p.noduri[i].y=p.y;
        p.noduri[i].legat=0;
    }
...
void linie(piesa p, int i)
{
    ...
    line(p.x+zoom1*x1, p.y+zoom2*y1, p.x+zoom1*x2, p.y+zoom2*y2);
}
...
void cerc(piesa p, int i)
{
    ...
    y1=p.descriere.y1[i];
    r=p.descriere.x2[i];
    circle(p.x+zoom1*x1, p.y+zoom2*y1, zoom1*r);
}
...
void dreptunghi(piesa p, int i)
{
    ...
    rectangle(p.x+zoom1*x1, p.y+zoom2*y1, p.x+zoom1*x2, p.y+zoom2*y2);
}
...
{
    setlinestyle(0, 0, 1);
    if (!strcmp(p.nume, "Bec") || !strcmp(p.nume, "LightBulb"))
        setcolor(rand()%15);
    else
    {
        if (selectie==2)
...
void zoomin(piesa Piese[], int contor, unsigned &zoom1, unsigned &zoom2)
{
    bool ok=1;
    for (i=0; i<contor; i++)
        if (!(Piese[i].x>=240+(zoom1+5)*1.6 && Piese[i].x<=760-(zoom1+5)*1.6 && Piese[i].y>=70+(zoom2+5)*0.6 && Piese[i].y<=630-(zoom2+5)*0.6)) ok=0;
    if (zoom1<=65 && ok && zoom2<=65)
    {
        zoom1+=5; /*zoom2+=10;*/
        draw_ecran_circ();
        for (i=0; i<=contor; i++)
...
{
    for (i=0; i<contor; i++)
        if (x>=Piese[i].x+zoom1*Piese[i].c1x && x<=Piese[i].x+zoom1+Piese[i].c2x && y>=Piese[i].y+zoom2*Piese[i].c1y && y<=Piese[i].y+zoom2*Piese[i].c2y)
            return i;
    return -1;
}
    ...
    do
    {
        ...
        if (ismouseclick(WM_LBUTTONDOWN) && !click)
        {
            ...
            if (!inf && x >= 800 && x <= 999 && y>=420 && y<=490)
                zoomin(Piese, contor, zoom1, zoom2);
            else if (!inf && x >= 800 && x <= 999 && y>=490 && y<=560)
                zoomout(Piese, contor, zoom1);
                ...
                alegePiesa(p, y);
            else if (!inf && selectie==1 && x>=240+zoom1*1.6 && x<=760-zoom1*1.6 && y>=70+zoom2*0.6 && y<=630-zoom2*0.6)
                punePiesa(p, x, y);
            ...
            else if (!inf && selectie==2 && x>=240+zoom1*1.55 && x<=760-zoom1*1.55 && y>=70+zoom2*0.5 && y<=630-zoom2*0.5)
